...
$\text{Алгоритм используется для факторизации числа n = pq.} g(x) = (x^2 + 1)mod n \\   используется для генерации  \text{псевдорандомной последовательности. }\\ \text{Выбирается начальное значение, например 2, и последовательность продолжается как }x_1 = g(2), x_2 = g(g(2))...$
